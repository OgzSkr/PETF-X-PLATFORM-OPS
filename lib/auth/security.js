...
export function validateSecurityConfig(config) {
  ...
  const allowSimpleToken = Boolean(config.panelAllowSimpleToken);
  if (
    isProduction
    && config.platformApiToken
    && config.platformApiToken.length < 24
    && !allowSimpleToken
  ) {
    errors.push('PLATFORM_API_TOKEN çok kısa — production için en az 24 karakter kullanın veya PANEL_ALLOW_SIMPLE_TOKEN=true ekleyin.');
  }
