...
import { PRODUCT_LINE } from '../marketnext/constants.js';

/**
 * Tek kanal kaynağı — operasyon, eşleştirme ve navigasyon buradan türetilir.
 * productLine: marketnext | marketplace | ecommerce
 */
export const CHANNELS = {
  ...
  getir: {
    ...
    label: 'Getir',
    shortLabel: 'Getir',
    route: '/marketnext/orders/getir',
    navTabId: 'getir',
    status: 'planned',
    scope: CHANNEL_SCOPE.ORDERS_PROFIT,
    productLine: PRODUCT_LINE.MARKETNEXT,
    matchingRole: 'sales',
    description: 'MarketNext — Getir Çarşı sipariş ve eşleştirme',
  ...
  'uber-eats': {
    ...
    shortLabel: 'Uber',
    route: '/marketnext/orders/uber-eats',
    navTabId: 'uber-eats',
    status: 'active',
    scope: CHANNEL_SCOPE.ORDERS_PROFIT,
    productLine: PRODUCT_LINE.MARKETNEXT,
    matchingRole: 'sales',
    description: 'MarketNext — TGO/Uber sipariş, eşleştirme ve BenimPOS satış',
    features: ['orders', 'profit', 'matching-catalog', 'matching-review', 'benimpos-sale']
  },
  yemeksepeti: {
    id: 'yemeksepeti',
    ...
    route: '/marketnext/orders/yemeksepeti',
    navTabId: 'yemeksepeti',
    status: 'active',
    scope: CHANNEL_SCOPE.ORDERS_PROFIT,
    productLine: PRODUCT_LINE.MARKETNEXT,
    matchingRole: 'sales',
    description: 'MarketNext — YS Hızlı Market sipariş ve katalog eşleştirme',
    features: ['orders', 'profit', 'matching-catalog', 'matching-review', 'benimpos-sale']
  },
  woocommerce: {
    id: 'woocommerce',
...
export function listMarketNextChannels() {
  return listChannels().filter((channel) => channel.productLine === PRODUCT_LINE.MARKETNEXT);
}

export function listActiveMarketNextChannels() {
  return listMarketNextChannels().filter((channel) => channel.status === 'active');
}

export function listMarketNextMatchingSalesChannels() {
  return listMatchingSalesChannels().filter((channel) => channel.productLine === PRODUCT_LINE.MARKETNEXT);
}

export function listActiveMarketNextMatchingSalesChannels() {
  return listMarketNextMatchingSalesChannels().filter((channel) => channel.status === 'active');
}
...
export function listPlatformNavTabs() {
  ...
  const navChannels = listActiveMarketNextChannels().map((channel) => ({
  ...
  return [
    { id: 'marketnext', href: '/marketnext', label: 'MarketNext' },
    ...channelTabs,
