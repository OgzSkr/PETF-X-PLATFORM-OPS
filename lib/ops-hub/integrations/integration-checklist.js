...
import { isYsConfigComplete, branchConfigToYsCfg, branchConfigToTgoCfg, isTgoConfigComplete } from './config-bridge.js';
...
export async function buildTrendyolGoSetupChecklist({
  ...
  const items = [
    ...
    statusItem(
      ...
      catalogCount > 0 ? { href: '/marketnext/matching?tab=uber-eats' } : {}
    ),
    statusItem(
      'matching',
      'Ürün eşleştirme',
    ...
    statusItem(
      ...
      'Stok push',
      'warn',
      'G2 kapısı — canlı stok gönderimi henüz simülasyon modunda.',
      { href: '/quick-commerce/health' }
    )
  ];
...
export async function buildGetirSetupChecklist({
  ...
  const items = [
    statusItem(
      ...
      { href: '/quick-commerce/integrations?channel=getir' }
    ),
    statusItem(
      'shop_id',
      'İşletme / Shop ID',
    ...
    statusItem(
      'connection_test',
      'Bağlantı testi',
      configMeta.lastTestOk === true ? 'done' : 'warn',
      configMeta.lastTestMessage || 'G3 — API credential gelene kadar shadow modda bekler.',
      { href: '/quick-commerce/health' }
    ),
    statusItem(
      'public_url',
