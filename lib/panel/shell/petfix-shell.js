...
import { listPanelModules, PANEL_MODULES, QC_PICKING_NAV_IDS } from '../nav-config.js';
...
function renderNavSection(module, activeModule, activeItem, { minimal = false } = {}) {
  if (minimal && module.id !== 'marketnext') return '';
  if (minimal && module.id === 'marketnext') {
  ...
  return renderNavGroup(module.label, module.items, activeModule, activeItem);
}
...
function renderNavGroup(label, items, activeModule, activeItem) {
  const links = items.map((item) => {
    ...
    return `<a class="${cls.trim()}" href="${escapeHtml(item.href)}" data-nav="${escapeHtml(item.id)}">
      ...
      <span class="pf-nav-label">${escapeHtml(item.label)}</span>${badge}
    </a>`;
  }).join('');
...
export function renderPetfixShell({
  ...
  const resolved = PANEL_MODULES[activeModule] || PANEL_MODULES.marketnext;
  ...
  <div class="pf-layout">
    ...
    <div class="pf-main-wrap">
      <header class="pf-topbar">
        <button type="button" id="pfNavToggle" class="pf-icon-btn pf-nav-toggle" aria-label="Menü">☰</button>
