...
import { createMarketNextOrdersService } from './services/marketnext-orders.js';
...
export async function startPlatform() {
  ...
  if (config.sqliteDualWrite) {
    try {
      ...
      log.warn(`SQLite başlangıç senkronu atlandı: ${error.message}`);
    }
  }
  ...
  const marketNextOrders = createMarketNextOrdersService({ channelOrders });
