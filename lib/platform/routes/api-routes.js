...
export async function handleApiRoutes(ctx) {
  ...
  if (request.method === 'POST' && url.pathname === '/api/product-matching/sync-master') {
    auth.assertAuthorized(request);
    await sendJson(response, await productMatching.syncMasterFromBenimpos());
    return true;
  }
  ...
  if (request.method === 'GET' && url.pathname === '/api/marketnext/orders') {
    auth.assertAuthorized(request);
    await sendJson(response, await marketNextOrders.listMarketNextOrders(url.searchParams));
    return true;
  }

  if (request.method === 'GET' && url.pathname === '/api/marketnext/orders/export') {
    auth.assertAuthorized(request);
    await marketNextOrders.sendMarketNextOrdersCsvExport(response, url.searchParams);
    return true;
  }
