...
export function createMarketNextOrdersService({ channelOrders }) {
  async function listMarketNextOrders(searchParams) {
  ...
  async function sendMarketNextOrdersCsvExport(response, searchParams) {
    const result = await listMarketNextOrders(searchParams);
  ...
  return { listMarketNextOrders, sendMarketNextOrdersCsvExport };
}
