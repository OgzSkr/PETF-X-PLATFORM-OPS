...
import { ingestTrendyolCatalogProducts } from '../../product-matching/channel-ingest/trendyol-marketplace.js';
import { ingestWooCommerceCatalogProducts } from '../../product-matching/channel-ingest/woocommerce.js';
import { ingestYemeksepetiCatalogProducts } from '../../product-matching/channel-ingest/yemeksepeti.js';
import { runAutoMatchForChannel, evaluateChannelToMasterMatch } from '../../product-matching/matcher.js';
...
import { buildMasterChannelPrices, channelSalePriceFromProduct } from '../../product-matching/master-channel-prices.js';
...
function resolveMatchingMode(platformEnv = {}) {
  const mode = envValue(process.env, platformEnv, 'PRODUCT_MATCHING_MODE', 'legacy').toLowerCase();
  return PRODUCT_MATCHING_MODES.includes(mode) ? mode : 'legacy';
}
...
function summarizeMasterAction(row, channelMappings = {}) {
  ...
  if (matchIssues.some((m) => m.status === 'auto_matched')) {
  ...
  return {
    ...
    needsDataQualityAction: dqIssues.length > 0 && matchIssues.length === 0
  };
}
...
export function createProductMatchingService() {
  ...
  async function getStatus() {
    ...
    return {
      ...
      woocommerceCatalogSyncedAt: pm.meta.channelIngest?.woocommerce?.ingestedAt || null,
      woocommerceCatalogProductCount: pm.meta.channelIngest?.woocommerce?.distinctProducts || null,
      ...
      trendyolCatalogSyncedAt: pm.meta.channelIngest?.['trendyol-marketplace']?.importedAt || null,
      trendyolCatalogProductCount: pm.meta.channelIngest?.['trendyol-marketplace']?.prepared || null,
  ...
  async function listMasterProducts(searchParams = {}) {
    ...
    const limit = Math.min(200, Math.max(25, Number(searchParams.get?.('limit') ?? searchParams.limit ?? 50) || 50));
    ...
    const tabCounts = computeMasterPoolTabCounts(db, pm.masterProducts);
    ...
    const profitValues = filtered
      .map((row) => masterProfitPctOnCost(row))
      .filter((value) => value != null);
    const avgProfitPct = profitValues.length
      ? Math.round((profitValues.reduce((sum, value) => sum + value, 0) / profitValues.length) * 10) / 10
      : null;
    ...
    const rows = filtered.slice(start, start + limit).map((row) => {
      ...
      return {
        ...
        channelMappings,
        channelMappingDetails: channelMappingDetailsForMaster(db, row.id),
        channelPrices: buildMasterChannelPrices(db, row),
        qualityFlags: masterQualityFlags(row),
        profitPct: masterProfitPctOnCost(row),
        actionSummary: summarizeMasterAction(row, channelMappings),
    ...
    return {
      ...
      summary: {
        ...
        inStock: filtered.filter((row) => Number(row.stock) > 0).length,
        withCost: filtered.filter((row) => Number(row.buyingPrice) > 0).length,
        missingMeta: filtered.filter((row) => !row.normalizedWeightG || !row.variantKey).length,
        needsAction: filtered.filter((row) => masterNeedsAction(db, row.id)).length,
        avgProfitPct,
        tabCounts
      }
    };
  ...
  async function syncMasterFromBenimpos() {
  ...
  async function syncTrendyolCatalogProducts() {
    const db = await readDb();
    const ingest = ingestTrendyolCatalogProducts(db);
    const pm = getProductMatching(db);

    const existing = new Map(
      pm.channelProducts
        .filter((cp) => cp.channelId === 'trendyol-marketplace')
        .map((cp) => [cp.channelProductId, cp])
    );
    ...
    for (const incoming of ingest.channelProducts) {
      if (existing.has(incoming.channelProductId)) {
        const current = existing.get(incoming.channelProductId);
        if (incoming.channelName && (!current.channelName || current.channelName === current.channelBarcode)) {
    ...
    pm.meta.channelIngest = pm.meta.channelIngest || {};
    pm.meta.channelIngest['trendyol-marketplace'] = ingest.summary;
    db.meta = db.meta || {};
    db.meta.updatedAt = new Date().toISOString();
    await writeDb(db);
  ...
  async function syncWooCommerceCatalogProducts(options = {}) {
    const ingest = await ingestWooCommerceCatalogProducts(options);
    const db = await readDb();
    const pm = getProductMatching(db);

    const existing = new Map(
      pm.channelProducts
        .filter((cp) => cp.channelId === 'woocommerce')
        .map((cp) => [cp.channelProductId, cp])
    );
    ...
    for (const incoming of ingest.channelProducts) {
      if (existing.has(incoming.channelProductId)) {
        ...
        if (incoming.wcEan) current.wcEan = incoming.wcEan;
        if (incoming.wcSku) current.wcSku = incoming.wcSku;
        if (incoming.channelPrice != null) {
          current.channelPrice = incoming.channelPrice;
        }
        current.updatedAt = new Date().toISOString();
        updated += 1;
      } else {
    ...
    pm.meta.channelIngest = pm.meta.channelIngest || {};
    pm.meta.channelIngest.woocommerce = ingest.summary;
    db.meta = db.meta || {};
    db.meta.updatedAt = new Date().toISOString();
    await writeDb(db);
  ...
  async function syncYemeksepetiCatalogProducts(options = {}) {
    ...
    pm.meta.channelIngest = pm.meta.channelIngest || {};
    pm.meta.channelIngest.yemeksepeti = ingest.summary;
    db.meta = db.meta || {};
    db.meta.updatedAt = new Date().toISOString();
    await writeDb(db);
  ...
  async function runAutoMatch(channelId = 'uber-eats') {
    const db = await readDb();
    const pm = getProductMatching(db);
    const summary = runAutoMatchForChannel(db, channelId);
    db.meta = db.meta || {};
    db.meta.updatedAt = new Date().toISOString();
    db.meta.lastAutoMatch = { channelId, at: new Date().toISOString(), ...summary };
  ...
  async function runAutoMatchAllChannels(options = {}) {
    const { MARKETNEXT_BUYBOX_CHANNEL_IDS } = await import('../../marketnext/constants.js');
  ...
  async function confirmMapping(payload) {
    ...
    const masterProductId = String(payload.masterProductId || '').trim();
    if (!channelProductId || !masterProductId) {
      throw new Error('channelProductId ve masterProductId zorunlu.');
    }

    const db = await readDb();
    const pm = getProductMatching(db);
    const result = applyManualConfirmMapping(db, pm, payload);
    if (!result.ok) {
  ...
  async function masterPoolBulkAction(payload = {}) {
    ...
    if (action === 'review') {
      return {
        ...
        redirect: '/products/inbox?queueMode=manual_review',
  ...
  async function removeMappingsBulk(payload) {
    ...
    for (const barcode of barcodes) {
      ...
      if (!mapping?.channelProductId) continue;
  ...
  function buildWorkbenchCompareFields(cp, master, channelPrice) {
    ...
    return [
      ...
      { key: 'barcode', label: 'Barkod', channel: cp.channelBarcode || '', master: master.benimposBarcode || '', state: compareFieldState(cp.channelBarcode, master.benimposBarcode) },
  ...
  function buildWorkbenchRow(db, pm, channel, cp, mapping) {
    let suggestedMasterProductId = cp.masterProductId;
    let suggestedMasterName = cp.masterProductName;
    ...
    return {
      ...
      priceDiffPct,
      priceDiffAbs,
      channelImageUrl: cp.imageUrl || cp.catalogImageUrl || null,
      parsedNameHints,
      mappingStatus: cp.mappingStatus,
      ...
      suggestionReason,
      suggestionRecovered,
      masterLinkConflict: cp.masterLinkConflict || false,
      confidenceScore: cp.masterLinkConflict ? 0 : confidenceScore,
  ...
  const WORKBENCH_LITE_CACHE_MS = 30000;
  ...
  return {
    ...
    syncTrendyolCatalogProducts,
    syncWooCommerceCatalogProducts,
