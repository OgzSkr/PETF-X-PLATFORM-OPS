...
import { isEcommerceChannel, isMarketNextBuyboxChannel, MARKETNEXT_BUYBOX_CHANNEL_IDS } from '../../marketnext/constants.js';
...
function resolveChannelPanelNav(channelId) {
  ...
  if (isMarketNextBuyboxChannel(channelId)) {
    return { activeModule: 'marketnext', activeItem: channelId };
  }
...
export function renderChannelOrdersPage({ channelId, auth, productMatchingMode = 'legacy' }) {
  ...
  const innerHtml = `
      <div class="orders-wrap">
        <section class="panel channel-orders-intro">
          ...
          <p class="platform-lead">Sipariş takibi ve kâr/zarar analizi. Maliyetler <a href="/marketnext/matching/mappings">Kanal Eşleşmeleri</a> sayfasından yönetilir (Trendyol Pazaryeri maliyetlerinden ayrı).</p>
        ...
        ${channelId === 'uber-eats' ? `<section class="orders-card orders-loss-products-panel" id="uberLossProductsPanel" hidden>
          <div class="orders-card-head">
            ...
            <div class="orders-loss-products-head-links">
              <a class="btn-detail ghost" href="/marketnext/matching?tab=uber-eats">Uber katalog</a>
              <a class="btn-detail ghost" href="/marketnext/matching?tab=master">Ana ürün havuzu</a>
            </div>
          </div>
  ...
  return wrapPanelPage({
    ...
    bodyHtml: innerHtml,
    stylesheets: ['/assets/styles.css?v=corp2', '/assets/orders.css?v=4'],
    scripts: ['/assets/channel-page.js?v=3', '/assets/orders.js?v=13']
  });
}
      ...
      <main class="platform-main">
        <section class="panel channel-empty-state">
          ...
          <div class="channel-empty-actions">
            ...
            <a class="btn btn-ghost" href="/marketnext">MarketNext</a>
          </div>
          <p class="channel-empty-hint muted">Maliyetler <a href="/marketnext/matching/mappings">Kanal Eşleşmeleri</a> sayfasından yönetilir.</p>
        </section>
      </main>
      <script id="bootstrap" type="application/json">${bootstrap}</script>`;
...
export function renderMarketNextProfitPage({ auth, productMatchingMode = 'legacy' }) {
  const bootstrap = jsonForHtml({
    authRequired: Boolean(auth.isEnabled()),
    multiChannel: true,
    channelId: null,
    channelLabel: 'MarketNext',
    productMatchingMode,
    productsPath: '/marketnext/matching/masters',
    matchingPath: '/marketnext/matching',
    orderDateTimezone: 'Europe/Istanbul',
    apiPath: '/api/marketnext/orders',
    exportPath: '/api/marketnext/orders/export',
    benimposSaleEnabled: false,
  ...
  const innerHtml = `
      <div class="orders-wrap pf-marketnext-profit">
        <section class="panel channel-orders-intro">
          <h2>Sipariş Kârlılığı</h2>
          <p class="platform-lead">Uber Eats, Yemeksepeti ve Getir siparişlerinin birleşik kâr/zarar analizi. Partner API ve webhook/Ops kaynakları birlikte gösterilir.</p>
          <nav class="orders-subnav" id="marketnextChannelFilters" role="tablist" aria-label="MarketNext kanalı">
            ${channelTabs.map((tab, index) => {
              if (tab.planned) {
                return `<span class="orders-subnav-tab orders-subnav-tab--planned" title="Yakında">${renderChannelLogo(tab.id, { size: 'xs' })}<span>${escapeHtml(tab.label)}</span><em class="orders-subnav-soon">yakında</em></span>`;
              }
              return `<button type="button" class="orders-subnav-tab orders-subnav-tab--channel${index === 0 ? ' active' : ''}" data-channel="${escapeHtml(tab.id)}" data-label="${escapeHtml(tab.label)}" role="tab" aria-selected="${index === 0 ? 'true' : 'false'}">${tab.id === 'all' ? '' : renderChannelLogo(tab.id, { size: 'xs' })}<span>${escapeHtml(tab.label)}</span><span class="orders-subnav-count" data-count-for="${escapeHtml(tab.id)}"></span></button>`;
            }).join('')}
          </nav>
          <p class="orders-source-note muted" id="ordersSourceNote" hidden></p>
        </section>
        ...
        <section class="orders-card" id="ordersListPanel">
          <div class="orders-card-head">
            ...
            <p class="muted">Kanal bazlı detay için satırdaki kanal linkini veya menüdeki Uber / Yemeksepeti sayfalarını kullanın.</p>
          </div>
          <div class="orders-toolbar">
            <button type="button" class="btn-brown" id="refreshOrders">Verileri Güncelle</button>
            <button type="button" class="btn-green" id="exportReport">Raporu İndir</button>
          ...
          <div class="orders-table-wrap" id="ordersTableWrap">
            <table class="orders-table">
              <thead>
                <tr>
                  <th data-sort="channelLabel">Kanal <span class="sort-icon">↕</span></th>
                  <th data-sort="orderNumber">Sipariş No <span class="sort-icon">↕</span></th>
                  <th data-sort="orderDateMs">Tarih <span class="sort-icon">↕</span></th>
                  <th data-sort="status">Durum <span class="sort-icon">↕</span></th>
                  <th data-sort="salesAmount">Tutar (₺) <span class="sort-icon">↕</span></th>
              ...
              <tbody id="ordersBody">
                <tr><td colspan="9" class="orders-loading">Yükleniyor…</td></tr>
              </tbody>
            </table>
          </div>
          <div class="orders-footer" id="ordersFooter">Yükleniyor…</div>
  ...
  return wrapPanelPage({
    title: 'MarketNext — Kâr/Zarar Raporu',
    activeModule: 'marketnext',
    activeItem: 'profit',
    auth,
    bodyClass: 'orders-page marketnext-profit-page pf-channel-page',
    bodyHtml: innerHtml,
    stylesheets: ['/assets/styles.css?v=corp2', '/assets/orders.css?v=4'],
    scripts: ['/assets/orders.js?v=13']
  });
}
