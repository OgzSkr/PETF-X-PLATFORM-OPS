...
import { listMarketNextSalesMatchingChannels } from '../../product-matching/constants.js';
...
export function createPlatformPages(auth, runtimeConfig = {}) {
    ...
    const innerHtml = `
        ...
        <main class="platform-main settings-main pf-admin-settings">
          ...
          <section class="panel settings-section">
            ...
            <p class="muted">OAuth token otomatik alınır (~2 saat). Katalog: <a href="/marketnext/matching?tab=yemeksepeti">Eşleştirme Merkezi</a> · Sipariş kârlılığı: <a href="/marketnext/orders/yemeksepeti">Yemeksepeti</a>.</p>
  ...
  function renderMatchingCenterPage({ defaultTab = 'master', focus = null } = {}) {
    const salesChannels = listMarketNextSalesMatchingChannels().map((channel) => ({
      ...channel,
    ...
    const activeItem = navItemByTab[defaultTab] || 'matching';
    const syncChannelOptions = salesChannels.map((channel) => ({
      id: channel.id,
    ...
    const bootstrap = jsonForHtml({
      ...
      defaultTab,
      focus,
      productLine: 'marketnext',
      matchingBase: MARKETNEXT_MATCHING,
      inboxBase: MARKETNEXT_INBOX
    });
    ...
    const innerHtml = `
        ...
        <header class="pf-page-header matching-hero-card">
          <div>
            <p class="pf-page-eyebrow">MarketNext · BenimPOS</p>
            <h1 id="matchingHeroTitle">Ürün Eşleştirme</h1>
            <p class="pf-page-lead" id="matchingHeroLead">Getir, Yemeksepeti ve TGO/Uber kanalları için BenimPOS eşleştirmesi. Pazaryeri ve WooCommerce bu ekranın dışındadır.</p>
          </div>
          <div class="pf-page-actions matching-hero-actions">
            ...
            <div class="matching-hero-menu">
              ...
              <div class="matching-hero-menu-panel" id="matchingHeroMenuPanel" hidden>
                ...
                <a class="matching-hero-menu-item matching-hero-menu-link" href="/marketnext/orders">Sipariş Kârlılığı</a>
        ...
        <p class="matching-sync-meta-line muted" id="matchingSyncMetaLine">Son sync: <span id="kpiMasterSync">—</span></p>
          ...
          <section class="matching-sync-panel" id="matchingSyncPanel" aria-label="Otomatik eşleştirme sync">
            ...
            <div class="matching-sync-channels" id="matchingSyncChannels" aria-label="Sync kanalları">
              ...
              ${syncChannelOptions.map((channel) => `
              <label class="matching-check"><input type="checkbox" name="syncChannel" value="${escapeHtml(channel.id)}" checked> ${escapeHtml(channel.label)}</label>`).join('')}
    ...
    return renderPetfixShell({
      title: 'MarketNext — Ürün Eşleştirme',
      ...
      topbarActionsHtml: `<a href="${MARKETNEXT_INBOX}" class="pf-btn pf-btn-ghost-sm">Gelen Kutusu</a><button type="button" class="pf-btn-primary" id="masterExportTopBtn">Dışa Aktar</button>`,
      stylesheets: [
      ...
      scripts: [
        ...
        '/assets/matching-center.js?v=82',
