...
export function proposeMatchForChannelProduct(channelProduct, masterProducts = []) {
  const barcode = normalizeBarcode(channelProduct.channelBarcode);
  if (!barcode) {
    return {
  ...
  const candidates = lookupMastersByBarcode(index, channelProduct.channelBarcode);
...
export function runAutoMatchForChannel(db, channelId = 'uber-eats') {
