...
import { listActiveMarketNextMatchingSalesChannels, channelHasFeature, getChannel } from '../channels/registry.js';
...
function poolHref(channelId, params = {}) {
  ...
  return `${MARKETNEXT_MATCHING}?${search.toString()}`;
}
...
export function resolveInboxHref(row = {}) {
  ...
  const base = `${MARKETNEXT_INBOX}?channelId=${encodeURIComponent(channelId)}`;
  ...
  if (missingMaster > 0 && missingMaster >= needsReview && missingMaster >= autoPending) {
    return `${base}&queueMode=missing_master`;
  }
  if (needsReview > 0 && needsReview >= autoPending) {
    return `${base}&queueMode=manual_review`;
  }
  if (autoPending > 0) {
    return `${base}&queueMode=high_confidence`;
  }
  return base;
}
...
function catalogIngestTime(pm, channelId) {
  ...
  if (channelId === 'woocommerce') {
    return pm.meta.channelIngest?.woocommerce?.ingestedAt || null;
  }
  if (channelId === 'yemeksepeti') {
    return pm.meta.channelIngest?.yemeksepeti?.ingestedAt || null;
  }
  if (channelId === 'trendyol-marketplace') {
    return pm.meta.channelIngest?.['trendyol-marketplace']?.importedAt || null;
  }
...
export function buildMatchingQueue(db, config = {}) {
  ...
  for (const channel of listActiveMarketNextMatchingSalesChannels()) {
    ...
    channels.push({
      ...
      hrefMissing: `${MARKETNEXT_INBOX}?channelId=${encodeURIComponent(channel.id)}&queueMode=missing_master`,
      hrefAutoMatched: `${MARKETNEXT_INBOX}?channelId=${encodeURIComponent(channel.id)}&queueMode=high_confidence`,
      hrefReview: `${MARKETNEXT_INBOX}?channelId=${encodeURIComponent(channel.id)}&queueMode=manual_review`
    });
  }
  ...
  if (queue.totals?.missingMaster > 0) {
    ...
    if (!channelCoversMissing) {
      items.push({
        ...
        href: '/products/inbox?queueMode=missing_master',
...
export function catalogChannelOpsConfig(channelId) {
  ...
  if (channel.productLine !== 'marketnext') return null;
  if (channelId === 'yemeksepeti') {
    return {
      channelId,
      label: channel.label,
