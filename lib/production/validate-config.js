...
export function validateProductionConfig(platformEnv = {}, processEnv = process.env) {
  ...
  const webhookVerifyDisabled = read(processEnv, platformEnv, 'WEBHOOK_VERIFY_DISABLED').toLowerCase();
  if (!webhookSecret && webhookVerifyDisabled !== 'true' && webhookVerifyDisabled !== '1') {
    missing.push('Yemeksepeti webhook secret (YEMEKSEPETI_WEBHOOK_SECRET) veya WEBHOOK_VERIFY_DISABLED=true');
  }
